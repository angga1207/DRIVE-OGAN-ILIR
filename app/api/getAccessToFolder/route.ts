...
export async function POST(request: NextRequest) {
    try {
        ...
        formData.append('slug', slug ?? 0);

        // Make request to actual server
        const res = await axios.post(`${ServerDomain}/getAccessToFolder`, formData, axiosConfig);

        return NextResponse.json(res.data);

    } catch (error: any) {
        console.error('Error in /api/getAccessToFolder:', error);
